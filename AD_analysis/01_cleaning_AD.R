...
# set paths
rawPathF <- "/home/gabriele/work/cbmc/scrnaseq/raw_data_new/566_20/"
rawPathM <- "/home/gabriele/work/cbmc/scrnaseq/raw_data_new/572_20/"
...
rawD[["3xTG_M"]] <- read.csv(paste0(rawPathM, "572_20_SampleTag04_mm_3xTG/572_20_SampleTag04_mm_3xTG_DBEC_MolsPerCell.csv"), skip=8, header=T, sep=",")
rawD[["WT_M"]] <- read.csv(paste0(rawPathM, "572_20_SampleTag03_mm_WT/572_20_SampleTag03_mm_WT_DBEC_MolsPerCell.csv"), skip=8, header=T, sep=",")
print(paste0(expN, " loaded"))
# analyse exp 566 AD female
rawD[["3xTG_F"]] <- read.csv(paste0(rawPathF, "566_20_SampleTag02_mm_3xTG/566_20_SampleTag02_mm_3xTG_DBEC_MolsPerCell.csv"), skip=8, header=T, sep=",")
rawD[["WT_F"]] <- read.csv(paste0(rawPathF, "566_20_SampleTag01_mm_WT/566_20_SampleTag01_mm_WT_DBEC_MolsPerCell.csv"), skip=8, header=T, sep=",")
...
antiPos <- grep("pAbO", rownames(abSce[["WT_M"]]))

# get all the info for AD
noAbSce[["3xTG_M"]] <- abSce[["3xTG_M"]][-antiPos, ]
# and WT
noAbSce[["WT_M"]] <- abSce[["WT_M"]][-antiPos, ]

# find antibodies and remove them from the dataset
antiPos <- grep("pAbO", rownames(abSce[["WT_F"]]))

# get all the info for AD
noAbSce[["3xTG_F"]] <- abSce[["3xTG_F"]][-antiPos, ]
# and WT
noAbSce[["WT_F"]] <- abSce[["WT_F"]][-antiPos, ]
...
for (n in names(noAbSce)) {
	...
	colData(cleanSce[[n]])$gender <- strsplit(n, "_")[[1]][2]
	colData(cleanSce[[n]])$conditionBYgender <- n
}
...
sharedGenes <- rownames(cleanSce[["WT_F"]])[which(rownames(cleanSce[["WT_F"]]) %in% rownames(cleanSce[["3xTG_M"]]))]
...
mergedSce <- cbind(cleanSce[["3xTG_M"]], cleanSce[["WT_M"]], cleanSce[["3xTG_F"]], cleanSce[["WT_F"]])
...
clust <- quickCluster(mergedSce, block=factor(mergedSce$conditionBYgender))
