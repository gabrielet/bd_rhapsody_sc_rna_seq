...
clusteredSce <- readRDS(paste0(saveIn, "QCed_data_with_clusters_", dimRedMet, "_", expN, ".Rds"))
...
tab_alldata_labels <- table(clusteredSce$fine_to_main, clusteredSce$condition)

# export the full table
write.xlsx(tab_alldata_labels, paste0(saveIn, "cluster_content_full_labels_ALL_DATASET_exp_", expN, "_SLM_ZERO_TRE.xlsx"), overwrite=T, colNames=F)
...
for (cT in seq(1, length(cellLab), by=1)){
	...
	tab_fine_to_main <- table(Assigned=clusteredSce$fine_to_main, Cluster=clusteredSce$slm_0.3, Condition=clusteredSce$condition)
	...
	# export both tables
	write.xlsx(eTabCNEAE, paste0(saveImg, "EAE_cluster_content_ALL_CLUSTERS_exp_", expN, "_", cellLab[cT], "_SLM_ZERO_TRE.xlsx"), overwrite=T, colNames=F)

	write.xlsx(eTabCNCTRL, paste0(saveImg, "CTRL_cluster_content_ALL_CLUSTERS_exp_", expN, "_", cellLab[cT], "_SLM_ZERO_TRE.xlsx"), overwrite=T, colNames=F)

	##################### FULL

	tab_full_labels <- table(Assigned=clusteredSce$pruned_fine, Cluster=clusteredSce$slm_0.3, Condition=clusteredSce$condition)
	...
	# export the full table
	write.xlsx(eTabcnFEAE, paste0(saveImg, "EAE_cluster_content_full_labels_ALL_CLUSTERS_exp_", expN, "_", cellLab[cT], "_SLM_ZERO_TRE.xlsx"), overwrite=T, colNames=F)

	write.xlsx(eTabcnFCTRL, paste0(saveImg, "CTRL_cluster_content_full_labels_ALL_CLUSTERS_exp_", expN, "_", cellLab[cT], "_SLM_ZERO_TRE.xlsx"), overwrite=T, colNames=F)
	...
	print(paste0(cellLab[cT], " are found in ", length(unique(clusteredSce$slm_0.3[cTPos])), " different clusters"))

	fR <- table(clusteredSce$slm_0.3[cTPos])
	...
	df <- data.frame(table(clusteredSce$slm_0.3, clusteredSce$fine_to_main, clusteredSce$condition))
	...
	# update label
	lbl <- lbl + 1
	png(paste0(saveImg, "04_0", lbl, "_", conD[2], "_cluster_composition_ALL_CLUSTERS_", expN, "_SLM_ZERO_TRE.png"), type="cairo", units="in", width=8, height=6, pointsize=12, res=300)
		plot(p)
	dev.off()
	...
	# update label
	lbl <- lbl + 1
	png(paste0(saveImg, "04_0", lbl, "_", conD[1], "_cluster_composition_ALL_CLUSTERS_", expN, "_SLM_ZERO_TRE.png"), type="cairo", units="in", width=8, height=6, pointsize=12, res=300)
		plot(p)
	dev.off()
	...
	write.table(with_row_names, paste0(saveIn, "counts_CONTE_GREZZE_CLUSTERING_GENERALE_exp_", expN, "_", cellLab[cT], "_SLM_ZERO_TRE.csv"), row.names=F, quote=F, sep="\t")
	#######################
	...
	write.table(with_row_names, paste0(saveIn, "logcounts_all_the_selected_cells_version_barbara_elena_eleonora_only_clustering_generale_PRIMA_DI_LOGNORMCOUNTS_", expN, "_", cellLab[cT], "_SLM_ZERO_TRE.csv"), row.names=F, quote=F, sep="\t")
	...
	write.table(with_row_names, paste0(saveIn, "logcounts_clustering_generale_version_barbara_elena_eleonora_only_ALL_CLUSTERS_DOPO_LOGNORMCOUNTS_", expN, "_", cellLab[cT], "_SLM_ZERO_TRE.csv"), row.names=F, quote=F, sep="\t")
...
################	
################	write.table(logCsCTRL, paste0(saveIn, "logcounts_ALL_CLUSTERS_CTRL_", expN, "_", cellLab[cT], "_SLM_ZERO_TRE.csv"), row.names=F, quote=F, sep="\t")
################	write.table(logCsEAE, paste0(saveIn, "logcounts_ALL_CLUSTERS_EAE_", expN, "_", cellLab[cT], "_SLM_ZERO_TRE.csv"), row.names=F, quote=F, sep="\t")
################	
################	########################################################################################################################################################
	...
	if (ncol(cTSce[[conD[1]]]) >= 10 & ncol(cTSce[[conD[2]]]) >= 10) {
		...
		# and export
#		write.table(sortedFinal, paste0(saveUpDown, "up_down_FindMarkers_ALL_CLUSTERS_", cellLab[cT], "_CELLS_", expN, "_SLM_ZERO_TRE.csv"), col.names=T, row.names=F, sep="\t")
		write.xlsx(sortedFinal, paste0(saveUpDown, "up_down_FindMarkers_ALL_CLUSTERS_", cellLab[cT], "_CELLS_", expN, "_SLM_ZERO_TRE.xlsx"), overwrite=TRUE)
	}
	
	# GABRIELA eae VERSUS ctrl
	
	# get expN only
	cTSubSetGC_only <- cTSubSet
	
	cTSce <- list()
	conD <- unique(cTSubSetGC_only$condition)

	if (ncol(cTSubSetGC_only[, which(cTSubSetGC_only$condition==conD[1])]) >= 10 & ncol(cTSubSetGC_only[, which(cTSubSetGC_only$condition==conD[2])]) >= 10) {
	
		# set CTRL as a reference, to compute fold change
		cTSubSetGC_only$condition <- relevel(as.factor(cTSubSetGC_only$condition), ref="CTRL")
		
		print("finding markers")
	
		# FINDMARKERS for DEGS
		markersG <- findMarkers(cTSubSetGC_only, groups=cTSubSetGC_only$condition, pval.type="any", direction="any")
		...
		# and export
		write.xlsx(sortedFinal, paste0(saveUpDown, "up_down_FindMarkers_ALL_CLUSTERS_eae_VS_ctrl_ONLY_", cellLab[cT], "_CELLS_", expN, "_SLM_ZERO_TRE.xlsx"), overwrite=TRUE)
	}
}
