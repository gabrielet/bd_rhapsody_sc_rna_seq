...
if (expN == "Onset") {
	...
	rawD[["CTRL"]] <- read.csv(paste0(raw_path_onset, "588_20_SampleTag05_mm_CTRL/588_20_SampleTag05_mm_CTRL_DBEC_MolsPerCell.csv"), skip=8, header=T, sep=",")
	rawD[["EAE"]] <- read.csv(paste0(raw_path_onset, "588_20_SampleTag06_mm_EAE/588_20_SampleTag06_mm_EAE_DBEC_MolsPerCell.csv"), skip=8, header=T, sep=",")

} else if (expN == "Chronic") {

	# analyse exp 589 EAE chronic
	rawD[["CTRL"]] <- read.csv(paste0(raw_path_chronic, "589_20_SampleTag07_mm_CTRL/589_20_SampleTag07_mm_CTRL_DBEC_MolsPerCell.csv"), skip=8, header=T, sep=",")
	rawD[["EAE"]] <- read.csv(paste0(raw_path_chronic, "589_20_SampleTag08_mm_EAE/589_20_SampleTag08_mm_EAE_DBEC_MolsPerCell.csv"), skip=8, header=T, sep=",")
...
mitoGenes <- read.csv("/home/gabriele/work/cbmc/scrnaseq/scrnaseq/mito_genes.csv", header=F, sep="\t")
...
antiPos <- grep("pAbO", rownames(abSce[["CTRL"]]))

# get all the info for the experiment
noAbSce[["EAE"]] <- abSce[["EAE"]][-antiPos, ]
noAbSce[["CTRL"]] <- abSce[["CTRL"]][-antiPos, ]
...
for (n in names(noAbSce)) {
	...
	png(paste0(saveImg, "01_0", lbl, "_filteredCells_", expN, ".png"), type="cairo", units="in", width=8, height=6, pointsize=12, res=300)
		plot(p)
	dev.off()
	lbl <- lbl + 1
	...
	png(paste0(saveImg, "01_0", lbl, "_geneExpression_", expN, ".png"), type="cairo", units="in", width=8, height=6, pointsize=12, res=300)
...
sharedGenes <- rownames(cleanSce[["CTRL"]])[which(rownames(cleanSce[["CTRL"]]) %in% rownames(cleanSce[["EAE"]]))]
...
# and merge
mergedSce <- cbind(cleanSce[["CTRL"]], cleanSce[["EAE"]])
...
png(paste0(saveImg, "01_0", lbl, "_combined_gene_expression_", expN, ".png"), type="cairo", units="in", width=8, height=6, pointsize=12, res=300)
...
png(paste0(saveImg, "01_0", lbl, "_norm_libSF_merged_", expN, ".png"), type="cairo", units="in", width=8, height=6, pointsize=12, res=300)
...
# and plot!
png(paste0(saveImg, "01_0", lbl, "_libSF_Deconv_", expN, ".png"), type="cairo", units="in", width=8, height=6, pointsize=12, res=300)
...
png(paste0(saveImg, "01_0", lbl, "_sumFactors_", expN, ".png"), type="cairo", units="in", width=8, height=6, pointsize=12, res=300)
	par(mfrow=c(1, 2))
	# sumFactors
...
# and plotting
png(paste0(saveImg, "01_0", lbl, "_librarySize_", expN, ".png"), type="cairo", units="in", width=8, height=6, pointsize=12, res=300)
