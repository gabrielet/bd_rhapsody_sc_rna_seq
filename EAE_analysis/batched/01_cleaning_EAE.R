...
# set paths
raw_path_onset <- "/home/gabriele/work/cbmc/scrnaseq/raw_data_new/588_20/"
raw_path_chronic <- "/home/gabriele/work/cbmc/scrnaseq/raw_data_new/589_20/"

raw_path <- "/home/gabriele/work/cbmc/scrnaseq/raw_data_new/"
expN <- "EAE_batched"

saveIn <- paste0(raw_path, "EAE_batched/")
...
# analyse exp 588 EAE onset
rawD[["CTRL_Onset"]] <- read.csv(paste0(raw_path_onset, "588_20_SampleTag05_mm_CTRL/588_20_SampleTag05_mm_CTRL_DBEC_MolsPerCell.csv"), skip=8, header=T, sep=",")
rawD[["EAE_Onset"]] <- read.csv(paste0(raw_path_onset, "588_20_SampleTag06_mm_EAE/588_20_SampleTag06_mm_EAE_DBEC_MolsPerCell.csv"), skip=8, header=T, sep=",")
print(paste0(expN, " loaded"))
# analyse exp 589 EAE chronic
rawD[["CTRL_Chronic"]] <- read.csv(paste0(raw_path_chronic, "589_20_SampleTag07_mm_CTRL/589_20_SampleTag07_mm_CTRL_DBEC_MolsPerCell.csv"), skip=8, header=T, sep=",")
rawD[["EAE_Chronic"]] <- read.csv(paste0(raw_path_chronic, "589_20_SampleTag08_mm_EAE/589_20_SampleTag08_mm_EAE_DBEC_MolsPerCell.csv"), skip=8, header=T, sep=",")
...
for (n in names(rawD)) {
	...
	rownames(transposed) <- colnames(rawD[[n]])[-1]
	...
	# EMPTY DROPLETS RETRIEVAL IS NOT PERFORMED (http://bioconductor.org/books/release/OSCA/droplet-processing.html)
	...
	# Also, see here: #https://support.bioconductor.org/p/123554/
	...
	# remove RIBOSOMAL AND RIKEN GENES AS DESCRIBED IN KIPNIS 2020, here https://bioinformatics.stackexchange.com/questions/3205/what-are-riken-genes
	
	# and here https://bioinformatics.stackexchange.com/questions/3197/how-to-filter-ribosomal-rna-from-scrna-seq-data){target="_blank"
	# and using this http://ribosome.med.miyazaki-u.ac.jp/rpg.cgi?mode=orglist&org=Mus%20musculus&type=m as reference

	# find riken genes
	riks <- grep("Rik", rownames(abSce[[n]]))
	#which percentage??
	print(paste0("percentage of RIKEN genes in ", n, " ", round((length(riks)/length(rownames(abSce[[n]])))*100, digits=2), "%"))

	# remove riken genes, if any
	if (length(riks) > 0) {
		abSce[[n]] <- abSce[[n]][-riks, ]
	}
	
	# and ribosomal proteins
	ribs <- grep("Rpl|rpl|rps|Rps|Lactb", rownames(abSce[[n]]))
	#which percentage??
	print(paste0("percentage of ribosomal genes in ", n, " ", round((length(ribs)/length(rownames(abSce[[n]])))*100, digits=2), "%"))
	
	# remove ribosomal genes, if any
	if (length(ribs) > 0) {
		abSce[[n]] <- abSce[[n]][-ribs, ]
	}
}
...
mitoGenes <- read.csv("/home/gabriele/work/cbmc/scrnaseq/scrnaseq/working_pipe/sc_analysis/mito_genes.csv", header=F, sep="\t")
...
antiPos <- grep("pAbO", rownames(abSce[["CTRL_Onset"]]))

# get all the info for Onset
noAbSce[["EAE_Onset"]] <- abSce[["EAE_Onset"]][-antiPos, ]
# and Chronic
noAbSce[["CTRL_Onset"]] <- abSce[["CTRL_Onset"]][-antiPos, ]

# find antibodies and remove them from the dataset
antiPos <- grep("pAbO", rownames(abSce[["CTRL_Chronic"]]))

# get all the info for Onset
noAbSce[["EAE_Chronic"]] <- abSce[["EAE_Chronic"]][-antiPos, ]
# and Chronic
noAbSce[["CTRL_Chronic"]] <- abSce[["CTRL_Chronic"]][-antiPos, ]
...
for (n in names(noAbSce)) {
	...
	png(paste0(saveImg, "01_0", lbl, "_filteredCells.png"), type="cairo", units="in", width=8, height=6, pointsize=12, res=300)
		plot(p)
	dev.off()
	lbl <- lbl + 1
	...
	png(paste0(saveImg, "01_0", lbl, "_geneExpression.png"), type="cairo", units="in", width=8, height=6, pointsize=12, res=300)
	...
	# add condition, i.e. CTRL or EAE, to colData
	colData(cleanSce[[n]])$condition <- strsplit(n, "_")[[1]][1]
	colData(cleanSce[[n]])$stage <- strsplit(n, "_")[[1]][2]
	colData(cleanSce[[n]])$conditionBYstage <- n
}
...
sharedGenes <- rownames(cleanSce[["CTRL_Chronic"]])[which(rownames(cleanSce[["CTRL_Chronic"]]) %in% rownames(cleanSce[["EAE_Onset"]]))]

for (n in names(cleanSce)) {
	cleanSce[[n]] <- cleanSce[[n]][which(rownames(cleanSce[[n]]) %in% sharedGenes), ]
}

# and merge #BATCHCORRECTION#BATCHCORRECTION#BATCHCORRECTION#BATCHCORRECTION#BATCHCORRECTION#BATCHCORRECTION#BATCHCORRECTION#BATCHCORRECTION#BATCHCORRECTION#BATCHCORRECTION#BATCHCORRECTION#BATCHCORRECTION#BATCHCORRECTION#BATCHCORRECTION#BATCHCORRECTION#BATCHCORRECTION#BATCHCORRECTION#BATCHCORRECTION#BATCHCORRECTION#BATCHCORRECTION#BATCHCORRECTION#BATCHCORRECTION#BATCHCORRECTION#BATCHCORRECTION#BATCHCORRECTION#BATCHCORRECTION#BATCHCORRECTION#BATCHCORRECTION#BATCHCORRECTION#BATCHCORRECTION
mergedSce <- cbind(cleanSce[["CTRL_Chronic"]], cleanSce[["CTRL_Onset"]], cleanSce[["EAE_Onset"]], cleanSce[["EAE_Chronic"]])
...
png(paste0(saveImg, "01_0", lbl, "_combined_gene_expression.png"), type="cairo", units="in", width=8, height=6, pointsize=12, res=300)
...
png(paste0(saveImg, "01_0", lbl, "_norm_libSF_merged.png"), type="cairo", units="in", width=8, height=6, pointsize=12, res=300)
...
clust <- quickCluster(mergedSce, block=factor(mergedSce$conditionBYstage))

saveRDS(object=clust, file=paste0(saveIn, "quickclustered_", expN, ".Rds"))
...
# and plot!
png(paste0(saveImg, "01_0", lbl, "_libSF_Deconv.png"), type="cairo", units="in", width=8, height=6, pointsize=12, res=300)
...
png(paste0(saveImg, "01_0", lbl, "_sumFactors_1.png"), type="cairo", units="in", width=8, height=6, pointsize=12, res=300)
	par(mfrow=c(1, 2))
	# sumFactors
...
# and plotting
png(paste0(saveImg, "01_0", lbl, "_librarySize_1.png"), type="cairo", units="in", width=8, height=6, pointsize=12, res=300)
