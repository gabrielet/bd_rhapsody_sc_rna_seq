#' ---
#' title: "Dimensionality reduction with size factors, initially designed for EAE only"
#' author: "gabrielet"
...
# set paths
# rawPath <- "/home/gabriele/cbmc/scrnaseq/raw_data_new/588_20/" ; expN <- "588" # EAE onset
 rawPath <- "/home/gabriele/cbmc/scrnaseq/raw_data_new/589_20/" ; expN <- "589" # EAE chronic
...
png(paste0(saveImg, "02_01_geneVar.png"), type="cairo", units="in", width=8, height=6, pointsize=12, res=300)
...
png(paste0(saveImg, "02_02_geneVar_no_weights.png"), type="cairo", units="in", width=8, height=6, pointsize=12, res=300)
	plot(fitGVar$mean, fitGVar$var, xlab="Mean of log-expression", ylab="Variance of log-expression")
	curve(fitGVar$trend(x), col=colorsPal[5], add=TRUE, lwd=2)
...
png(paste0(saveImg, "02_03_techNoise.png"), type="cairo", units="in", width=8, height=6, pointsize=12, res=300)
...
png(paste0(saveImg, "02_04_techNoise_blocked.png"), type="cairo", units="in", width=8, height=6, pointsize=12, res=300)
	par(mfrow=c(1,2))
...
set.seed(131)
# run tsne
denoisedSce <- runTSNE(denoisedSce, use_dimred="PCA", perplexity=30)
# denoisedSce <- runTSNE(denoisedSce, use_dimred="PCA", perplexity=40)
# and plot
png(paste0(saveImg, "02_05_tSNE_denoised.png"), type="cairo", units="in", width=8, height=6, pointsize=12, res=300)
grid.arrange(
	scater::plotReducedDim(denoisedSce, dimred="TSNE", colour_by="condition") + theme(legend.position="right") + labs(title="By condition") + guides(color=guide_legend("Condition")),
	scater::plotReducedDim(denoisedSce, dimred="TSNE", colour_by="pruned_main") + theme(legend.position="right") + labs(title="By cell type") + guides(color=guide_legend("Cell type")) ,
	ncol=2)
dev.off()
...
saveRDS(object=denoisedSce, file=paste0(saveIn, "denoised_data_", expN, ".Rds"))
