...
cTypes <- c("Tgd", "Neutrophils", "Bcell")
...
for (cellType in cTypes) {
	...
	for (cS in clustS) {
		...
		if (length(unique(optOne$fine_to_main)) == 1) {
			...
			if (ncol(cTSubSet) > 0) {
				...
				optOneGenes <- optionOne(cTSubSet, expN, saveUpDown, personalT, testingExp, cellType, thresholD)
				...
				if (length(optOneGenes)==0) {
					...
					ranksVals <- gseaMyData(cTSubSet, testingExp, rN, saveGSEA, saveGes, savePages, expN, paste0(cellType, "_cluster_", cS))
		...
		if (ncol(cTSubSet) > 0) {
			...
			optTwoGenes <- optionTwo(cTSubSet, expN, NULL, saveUpDown, personalT, testingExp, cellType, mergedAB, thresholD)
	...
	if (ncol(cTSubSet) > 0) {
		...
		optThreeGenes <- optionThree(cTSubSet, expN, saveUpDown, personalT, testingExp, cellType, thresholD)
		...
		if (length(optThreeGenes)==0) {
			...
			rN <- paste0(saveImg, "04_0", lbl, "_", testingExp[1])
			# run enrichment on cTSubset
			ranksVals <- gseaMyData(cTSubSet, testingExp, rN, saveGSEA, saveGes, savePages, expN, paste0("ALL_", cellType, "_CELLS"))
			# export rankings to perform later enrichment
			rTab <- cbind.data.frame(gene=names(ranksVals), foldChange=ranksVals)
