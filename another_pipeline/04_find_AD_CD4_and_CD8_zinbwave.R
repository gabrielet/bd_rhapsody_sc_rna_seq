#' ---
#' title: "Analysing CD8 cells in AD exps"
#' author: "gabrielet"
...
for (exP in expS) {
	...
	cellType <- c("T.8|T.CD8", "CD4")
	# cType for directories and file names
	cType <- c("CD8", "CD4")
	
	# how many cells type?
	manyCells <- length(cType)
	
	for (cS in seq(1, length(manyCells, by=1))) {
		print("")
		print(paste0("analysing ", cType[cS], " for exp ", expN, " ", sex, " mice, reduced with Zinbwave"))
		print("")
		
		# then do everything
		saveIn <- paste0(rawPath, "results/")
		saveImg <- paste0(saveIn, "figures/analysis_", cType[cS], "/")
		saveUpDown <- paste0(saveIn, "enrichment_", cType[cS], "/UpDown/")
		saveRanks <- paste0(saveIn, "enrichment_", cType[cS], "/ranks/")
		...
		# export the table
		write.table(eTabCN, paste0(saveImg, "cluster_content_exp_", expN, "_", cType[cS], ".csv"), quote=F, row.names=F, col.names=F, sep="\t")
		write.xlsx(eTabCN, paste0(saveImg, "cluster_content_exp_", expN, "_", cType[cS], ".xlsx"), colNames=F)
		...
		write.table(eTabcnF, paste0(saveImg, "cluster_content_full_labels_exp_", expN, "_", cType[cS], ".csv"), quote=F, row.names=F, col.names=F, sep="\t")
		write.xlsx(eTabcnF, paste0(saveImg, "cluster_content_full_labels_exp_", expN, "_", cType[cS], ".xlsx"), colNames=F)
		...
		# update label
		lbl <- lbl + 1
		png(paste0(saveImg, "04_0", lbl, "_fine_to_main_clusters_matrix.png"), type="cairo", units="in", width=9, height=15, pointsize=12, res=300)
		pheatmap(log2(tab_fine_to_main+10), color=colorRampPalette(c(colorsPal[1], colorsPal[6]))(101))
		dev.off()
		lbl <- lbl + 1
		png(paste0(saveImg, "04_0", lbl, "_full_clusters_matrix.png"), type="cairo", units="in", width=9, height=15, pointsize=12, res=300)
		...
		# update label
		lbl <- lbl + 1
		png(paste0(saveImg, "04_0", lbl, "_clusters_", expN, ".png"), type="cairo", units="in", width=24, height=18, pointsize=12, res=300)
		grid.arrange(
		...
		# search for cellType
		cTPos <- grep(cellType[cS], clusteredSce$pruned_fine)

		print(paste0(cType[cS], " are found in ", length(unique(clusteredSce$slm_1.5[cTPos])), " different clusters"))
		...
		clusteredSce$plot_labs[cTPos] <- cType[cS]

		# plotting tSNE using cluster colours and cell label colours, side by side
		# update label
		lbl <- lbl + 1
		png(paste0(saveImg, "04_0", lbl, "_clusters_only_", cType[cS], ".png"), type="cairo", units="in", width=12, height=9, pointsize=12, res=300)
		grid.arrange(
		...
		for (cS in clustS) {
			...
			print(table(optOne$pruned_fine))
			
			print(paste0("######## SEARCHING FOR ", cType[cS], " IN CLUSTER ", cS, " ########"))

			# now find cellType only in cS
			cTPos <- grep(cellType[cS], optOne$pruned_fine)
			...
			if (ncol(cTSubSet) > 0) {
				...
				optOneGenes <- optionOne(cTSubSet, expN, saveUpDown, personalT, conD, cType[cS], thresholD)
				...
				if (length(optOneGenes)==0) {
					...
					ranksVals <- gseaMyData(cTSubSet, conD, rN, saveGSEA, saveGes, savePages, expN, paste0(cType[cS], "_cluster_", cS))
					# export rankings to perform later enrichment
					rTab <- cbind.data.frame(gene=names(ranksVals), foldChange=ranksVals)
					write.table(rTab, paste0(saveRanks, "rankings_genes_for_", cType[cS], "_cluster_", cS, ".csv"), col.names=T, row.names=F, sep="\t")
				}
			} else {
				print(paste0("not enough cells to compare the two conditions for ", cType[cS], " in cluster ", cS))
			}
			...
			if (ncol(cTSubSet) > 0) {
				...
				optTwoGenes <- optionTwo(cTSubSet, expN, saveAB, saveUpDown, personalT, conD, cType[cS], mergedAB, thresholD)
		...
		print(paste0("######## SEARCHING FOR ", cType[cS], " IN THE WHOLE DATASET ########"))
		...
		if (ncol(cTSubSet) > 0) {
			...
			optThreeGenes <- optionThree(cTSubSet, expN, saveUpDown, personalT, conD, cType[cS], thresholD)
			...
			if (length(optThreeGenes)==0) {
				...
				rN <- paste0(saveImg, "04_0", lbl, "_", conD[1])
				# run enrichment on cTSubset
				ranksVals <- gseaMyData(cTSubSet, conD, rN, saveGSEA, saveGes, savePages, expN, paste0("ALL_", cType[cS], "_CELLS"))
				# export rankings to perform later enrichment
				rTab <- cbind.data.frame(gene=names(ranksVals), foldChange=ranksVals)
				write.table(rTab, paste0(saveRanks, "rankings_genes_for_ALL_", cType[cS], "_CELLS.csv"), col.names=T, row.names=F, sep="\t")
			}
		} else {
