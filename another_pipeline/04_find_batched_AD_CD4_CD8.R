...
# set paths
rawPath <- "/home/gabriele/cbmc/scrnaseq/raw_data_new/AD_batched/"
...
for (cellType in cTypes) {
	...
	print("")
	print(paste0("analysing ", cType, " for batched experiments reduced with Zinbwave"))
	print("")
	...
	#saveGSEA <- paste0(saveIn, "enrichment_", cType, "/GSEA/")
	#saveGes <- paste0(saveIn, "enrichment_", cType, "/GSEA/enrichmentScore/")
	#savePages <- paste0(saveIn, "enrichment_", cType, "/abstracts/")
	saveRanks <- paste0(saveIn, "enrichment_", cType, "/ranks/")
	...
	# load zinbwaved data
	clusteredSce <- readRDS(paste0(saveIn, paste0("slm_clustered_data_batched_K_20_top_1000.Rds")))
	...
	# export the table
	write.table(eTabCN, paste0(saveImg, "cluster_content_exp_", expN, "_", cType, ".csv"), quote=F, row.names=F, col.names=F, sep="\t")
	write.xlsx(eTabCN, paste0(saveImg, "cluster_content_exp_", expN, "_", cType, ".xlsx"), colNames=F)
	...
	write.table(eTabcnF, paste0(saveImg, "cluster_content_full_labels_exp_", expN, "_", cType, ".csv"), quote=F, row.names=F, col.names=F, sep="\t")
	write.xlsx(eTabcnF, paste0(saveImg, "cluster_content_full_labels_exp_", expN, "_", cType, ".xlsx"), colNames=F)

	# get data set testing condition.
	# for AD, the conditions that need to be compared are WT (male+female) versus 3xTG (male+female)
	testingExp <- unique(clusteredSce$condition)
	...
	# search for cellType
	cTPos <- grep(cellType, clusteredSce$pruned_fine)

	print(paste0(cType, " are found in ", length(unique(clusteredSce$slm_1.5[cTPos])), " different clusters"))
	...
	clusteredSce$plot_labs[cTPos] <- cType

	# plotting tSNE using cluster colours and cell label colours, side by side
	# update label
	lbl <- lbl + 1
	png(paste0(saveImg, "04_0", lbl, "_clusters_only_", cType, ".png"), type="cairo", units="in", width=12, height=9, pointsize=12, res=300)
	grid.arrange(
	...
	for (cS in clustS) {
		...
		if (length(unique(optOne$pruned_fine)) == 1) {
			...
			print(paste0("######## SEARCHING FOR ", cType, " IN CLUSTER ", cS, " ########"))

			# now find cellType only in cS
			cTPos <- grep(cellType, optOne$pruned_fine)
			...
			if (ncol(cTSubSet) > 0) {
				...
				optOneGenes <- optionOne(cTSubSet, expN, saveUpDown, personalT, testingExp, cType, thresholD)
				...
				if (length(optOneGenes)==0) {
					...
					ranksVals <- gseaMyData(cTSubSet, testingExp, rN, saveGSEA, saveGes, savePages, expN, paste0(cType, "_cluster_", cS))
					# export rankings to perform later enrichment
					rTab <- cbind.data.frame(gene=names(ranksVals), foldChange=ranksVals)
					write.table(rTab, paste0(saveRanks, "rankings_genes_for_", cType, "_cluster_", cS, ".csv"), col.names=T, row.names=F, sep="\t")
				}
			} else {
				print(paste0("not enough cells to compare the two conditions for ", cType, " in cluster ", cS))
			}
		}
		#############################################################################################
		...
		if (ncol(cTSubSet) > 0) {
			...
			optTwoGenes <- optionTwo(cTSubSet, expN, NULL, saveUpDown, personalT, testingExp, cType, mergedAB, thresholD)
	...
	print(paste0("######## SEARCHING FOR ", cType, " IN THE WHOLE DATASET ########"))
	...
	if (ncol(cTSubSet) > 0) {
		...
		optThreeGenes <- optionThree(cTSubSet, expN, saveUpDown, personalT, testingExp, cType, thresholD)
		...
		if (length(optThreeGenes)==0) {
			...
			rN <- paste0(saveImg, "04_0", lbl, "_", testingExp[1])
			# run enrichment on cTSubset
			ranksVals <- gseaMyData(cTSubSet, testingExp, rN, saveGSEA, saveGes, savePages, expN, paste0("ALL_", cType, "_CELLS"))
			# export rankings to perform later enrichment
			rTab <- cbind.data.frame(gene=names(ranksVals), foldChange=ranksVals)
			write.table(rTab, paste0(saveRanks, "rankings_genes_for_ALL_", cType, "_CELLS.csv"), col.names=T, row.names=F, sep="\t")
		}
	} else {
