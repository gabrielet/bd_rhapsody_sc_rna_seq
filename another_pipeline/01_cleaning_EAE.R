...
if (expN == "588") {
	...
	rawD[["CTRL"]] <- read.csv(paste0(rawPath, "588_20_SampleTag05_mm_CTRL/588_20_SampleTag05_mm_CTRL_DBEC_MolsPerCell.csv"), skip=8, header=T, sep=",")
	rawD[["EAE"]] <- read.csv(paste0(rawPath, "588_20_SampleTag06_mm_EAE/588_20_SampleTag06_mm_EAE_DBEC_MolsPerCell.csv"), skip=8, header=T, sep=",")
	# import cells information file
	cInfo <- read.csv(paste0(rawPath, "Combined_588_20_Expression_Data.st"), sep="\t", skip=9, header=T, stringsAsFactors=F)
} else if (expN == "589") {
	print(paste0(expN, " loaded"))
	# analyse exp 589 EAE chronic
	rawD[["CTRL"]] <- read.csv(paste0(rawPath, "589_20_SampleTag07_mm_CTRL/589_20_SampleTag07_mm_CTRL_DBEC_MolsPerCell.csv"), skip=8, header=T, sep=",")
	rawD[["EAE"]] <- read.csv(paste0(rawPath, "589_20_SampleTag08_mm_EAE/589_20_SampleTag08_mm_EAE_DBEC_MolsPerCell.csv"), skip=8, header=T, sep=",")
	# import cells information file
	cInfo <- read.csv(paste0(rawPath, "Combined_589_20_Expression_Data.st"), sep="\t", skip=9, header=T, stringsAsFactors=F)
}
...
abOnly[["EAE"]] <- abSce[["EAE"]][antiPos, ]
# and CTRL
noAbSce[["CTRL"]] <- abSce[["CTRL"]][-antiPos, ]
abOnly[["CTRL"]] <- abSce[["CTRL"]][antiPos, ]
...
assay(abOnly[["CTRL"]], "centeredLR") <- clr(as.matrix(assay(abOnly[["CTRL"]], "counts")))
assay(abOnly[["EAE"]], "centeredLR") <- clr(as.matrix(assay(abOnly[["EAE"]], "counts")))
...
for (n in names(noAbSce)) {
	...
	# finally, add condition, i.e. CTRL or EAE, to colData
...
rowData(cleanSce[["EAE"]]) <- rowData(cleanSce[["CTRL"]])
# and merge
mergedSce <- cbind(cleanSce[["EAE"]], cleanSce[["CTRL"]])
